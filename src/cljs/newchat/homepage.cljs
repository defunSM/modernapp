...
(defn home [timer-str]
  [:div [:center [:h2 "SMchat"]]
       ...
       [:br] [:br] [:br]
       [:center [:img {:src "http://imgur.com/StGANnEl.png" :class "imgtilt"}]]
   ...
   [:center [:h3 "A pathfinding drawing canvas."]]
   [:br ] [:br]
   [:center [:div1 [:a {:href "/pathfinder"} "Pathfinder"]]]
   [:br]    [:br]
   [:center [:img {:src "http://imgur.com/qz1iGFAl.png" :class "imgtilt"}]]
   [:br][:br][:br]
